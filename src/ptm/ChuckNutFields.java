...
public class ChuckNutFields extends javax.swing.JFrame {
Connection con;
 ...
 public void tbl() {
        try {
            pst = con.prepareStatement("select * from `chucknut`");
            rs = pst.executeQuery();
            ResultSetMetaData rsd = rs.getMetaData();
            d = (DefaultTableModel) tblcn.getModel();
            d.setRowCount(0);
            while (rs.next()) {
                Vector v = new Vector();
            ...
            Logger.getLogger(ChuckNutFields.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
 public void Connect() {
        try {
    ...
    private void initComponents() {
        ...
        getContentPane().add(jPanel5, new org.netbeans.lib.awtextra.AbsoluteConstraints(1110, 70, 130, 630));
        ...
        tblcn.setFont(new java.awt.Font("Times New Roman", 0, 14)); // NOI18N
        ...
        tblcn.setShowHorizontalLines(true);
        tblcn.setShowVerticalLines(true);
    ...
    private void txtsearchKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtsearchKeyReleased
          ...
          try {
              if(txt.matches("^[0-9]+$")){
            String query="SELECT * FROM `chucknut` Where `Model No`="+txt;
                pst=con.prepareStatement(query);
              rs=pst.executeQuery();
              tblcn.setModel(DbUtils.resultSetToTableModel(rs));
              }
            else if (txt.matches("^[a-zA-Z0-9]+$")) {
    // Search by ID
    String query = "SELECT * FROM `chucknut` WHERE `Id` = '" + txt + "'";
    pst = con.prepareStatement(query);
              rs=pst.executeQuery();
              tblcn.setModel(DbUtils.resultSetToTableModel(rs));
              }
              ...
              {
              String query="SELECT * FROM `chucknut` Where `Name` LIKE '%"+txt+"%'";
                pst=con.prepareStatement(query);
              rs=pst.executeQuery();
              tblcn.setModel(DbUtils.resultSetToTableModel(rs));
              }
               if (txt.isEmpty()) {
    String query = "SELECT * FROM `chucknut`";
    pst = con.prepareStatement(query);
    rs = pst.executeQuery();
    tblcn.setModel(DbUtils.resultSetToTableModel(rs));}
            } catch (SQLException ex) {
                Logger.getLogger(ChuckNut.class.getName()).log(Level.SEVERE, null, ex);
            }
    ...
    private void btnaddMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnaddMouseClicked
        try {
            ...
            pst = con.prepareStatement("INSERT INTO `chucknut`(`Id`,`Name`, `Brand`, `Model No`, `Whole Sale`, `R Sale`, `Cost Price`, `Total Quantity`, `Show Room`, `Godam`, `Total Cost`) VALUES (?,?,?,?,?,?,?,?,?,?,?)");
pst.setString(1, Id);
 ...
 String Id = txtid.getText();
            ...
            int TotalQuantity = Integer.parseInt(txtsr.getText()) + Integer.parseInt(txtgodam.getText());
            Double TotalCost = Double.parseDouble(txtcp.getText()) * TotalQuantity;
            pst = con.prepareStatement("UPDATE `chucknut` SET `Id`=?,`Name`=?,`Brand`=?,`Whole Sale`=?,`R Sale`=?,`Cost Price`=?,`Total Quantity`=?,`Show Room`=?,`Godam`=?,`Total Cost`=? WHERE `Model No`=?");
            pst.setString(1, Id);
            pst.setString(2, ItemName);
            pst.setString(3, Brand);
    ...
    private void tblcnMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblcnMouseClicked
         d = (DefaultTableModel) tblcn.getModel();
        int sIndex = tblcn.getSelectedRow();
...
System.out.println("Button clicked");  
        int selectedRow = tblcn.getSelectedRow();
        if (selectedRow != -1) { // A row is selected  
            String id = tblcn.getValueAt(selectedRow, 0).toString();
            String itemName = tblcn.getValueAt(selectedRow, 1).toString();
            double price = Double.parseDouble(tblcn.getValueAt(selectedRow, 10).toString());
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(ChuckNutFields.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new ChuckNutFields().setVisible(true);
            }
        });
    }

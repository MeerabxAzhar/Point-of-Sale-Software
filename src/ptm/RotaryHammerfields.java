...
public class RotaryHammerfields extends javax.swing.JFrame {
Connection con;
 ...
 public void tbl() {
        try {
            pst = con.prepareStatement("select * from `rotary hammer`");
            rs = pst.executeQuery();
            ResultSetMetaData rsd = rs.getMetaData();
            d = (DefaultTableModel) tblrh.getModel();
            d.setRowCount(0);
            while (rs.next()) {
                Vector v = new Vector();
            ...
            Logger.getLogger(RotaryHammerfields.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
 public void Connect() {
        try {
    ...
    private void initComponents() {
        ...
        name.setText("Name");
        jPanel2.add(name, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 7, 70, 20));
        ...
        jPanel2.add(details, new org.netbeans.lib.awtextra.AbsoluteConstraints(260, 90, 60, -1));
        ...
        jPanel2.add(txtdetails, new org.netbeans.lib.awtextra.AbsoluteConstraints(330, 90, 160, -1));

        tblrh.setFont(new java.awt.Font("Times New Roman", 0, 14)); // NOI18N
        ...
        tblrh.setAutoResizeMode(javax.swing.JTable.AUTO_RESIZE_ALL_COLUMNS);
        tblrh.setShowHorizontalLines(true);
        tblrh.setShowVerticalLines(true);
        ...
        jPanel2.add(jScrollPane2, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 120, 1000, 440));
        ...
        Id.setText("Id");
        jPanel2.add(Id, new org.netbeans.lib.awtextra.AbsoluteConstraints(510, 90, 40, 20));
    ...
    private void txtsearchKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtsearchKeyReleased
          ...
          try {
              if(txt.matches("^[0-9]+$")){
            String query="SELECT * FROM `rotary hammer` Where `Model No`="+txt;
    ...
    // Search by ID
    String query = "SELECT * FROM `rotary hammer` WHERE `Id` = '" + txt + "'";
    pst = con.prepareStatement(query);
              rs=pst.executeQuery();
              tblrh.setModel(DbUtils.resultSetToTableModel(rs));
              }
              ...
              {
              String query="SELECT * FROM `rotary hammer` Where `Name` LIKE '%"+txt+"%'";
                pst=con.prepareStatement(query);
              rs=pst.executeQuery();
              tblrh.setModel(DbUtils.resultSetToTableModel(rs));
              }
               if (txt.isEmpty()) {
    String query = "SELECT * FROM `rotary hammer`";
    pst = con.prepareStatement(query);
    rs = pst.executeQuery();
    tblrh.setModel(DbUtils.resultSetToTableModel(rs));}
          } catch (SQLException ex) {
    ...
    private void tblrhMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblrhMouseClicked
         d = (DefaultTableModel) tblrh.getModel();
        int sIndex = tblrh.getSelectedRow();
        ...
        txtgodam.setText(d.getValueAt(sIndex, 10).toString());
         txtdetails.setText(d.getValueAt(sIndex, 11).toString());
    ...
    private void btnaddMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnaddMouseClicked
         try {
            ...
            pst = con.prepareStatement("INSERT INTO `rotary hammer`(`Id`, `Name`, `Model No`, `Company`, `Brand`, `Cost Price`, `Whole Price`, `Retail Price`, `Total Quantity`, `Show Room`, `Godam`, `Total Cost`, `Details`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)");
pst.setString(1, Id);
 ...
 String Id = txtid.getText();
            ...
            int TotalQuantity = Integer.parseInt(txtsr.getText()) + Integer.parseInt(txtgodam.getText());
            Double TotalCost = Double.parseDouble(txtcp.getText()) * TotalQuantity;
            pst = con.prepareStatement("UPDATE `rotary hammer` SET `Id`=?,`Name`=?,`Company`=?,`Brand`=?,`Cost Price`=?,`Whole Price`=?,`Retail Price`=?,`Total Quantity`=?,`Show Room`=?,`Godam`=?,`Total Cost`=?,`Details`=? WHERE `Model No`=?");
           pst.setString(1, Id);
            pst.setString(2, ItemName);
             pst.setString(3, Company);
            pst.setString(4, Brand);
            pst.setString(13, ModelNo);
...
System.out.println("Button clicked");  
        int selectedRow = tblrh.getSelectedRow();
        if (selectedRow != -1) { // A row is selected  
            String id = tblrh.getValueAt(selectedRow, 0).toString();
            String itemName = tblrh.getValueAt(selectedRow, 1).toString();
            double price = Double.parseDouble(tblrh.getValueAt(selectedRow, 10).toString());
...
ot.setVisible(true);          // TODO add your handling code here:
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(RotaryHammerfields.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new RotaryHammerfields().setVisible(true);
            }
        });
    }

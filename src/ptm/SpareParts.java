...
Connection con;
 ...
 public void tbl() {
        try {
            pst = con.prepareStatement("select * from `spare parts`");
            rs = pst.executeQuery();
            ResultSetMetaData rsd = rs.getMetaData();
            d = (DefaultTableModel) tblsp.getModel();
            d.setRowCount(0);
            while (rs.next()) {
                Vector v = new Vector();
                ...
                v.add(rs.getString("Core.Dia"));
                v.add(rs.getString("Core.Length"));
                v.add(rs.getString("B.B.Size"));
                v.add(rs.getString("F.B.D"));
                v.add(rs.getString("C.B.D"));
                v.add(rs.getString("Shaft"));
                v.add(rs.getString("Shaft.Dia"));
                v.add(rs.getString("Teeth"));
                d.addRow(v);
            }
             TableColumn column = tblsp.getColumnModel().getColumn(5);
 ...
 public void Connect() {
    ...
    private void initComponents() {
        ...
        tblsp.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Id", "Name", "Model No", "Company", "Brand", "C.P", "W.P", "R.P", "T.Q", "S.R", "G.D", "T.C", "Core.Dia", "Core.Length", "B.B.Size", "F.B.D", "C.B.D", "Shaft", "Shaft.Dia", "Teeth"
            }
        ));
        tblsp.setAutoResizeMode(javax.swing.JTable.AUTO_RESIZE_ALL_COLUMNS);
        tblsp.setShowHorizontalLines(true);
        tblsp.setShowVerticalLines(true);
    ...
    private void txtsearchKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtsearchKeyReleased
        String txt=txtsearch.getText();
          try {
              if(txt.matches("^[0-9]+$")){
            String query="SELECT * FROM `spare parts` Where `Model No`="+txt;
                pst=con.prepareStatement(query);
              rs=pst.executeQuery();
              tblsp.setModel(DbUtils.resultSetToTableModel(rs));
              }
            else if (txt.matches("^[a-zA-Z0-9]+$")) {
    // Search by ID
    String query = "SELECT * FROM `spare parts` WHERE `Id` = '" + txt + "'";
    pst = con.prepareStatement(query);
              rs=pst.executeQuery();
              tblsp.setModel(DbUtils.resultSetToTableModel(rs));
              }
              ...
              {
              String query="SELECT * FROM `spare parts` Where `Name` LIKE '%"+txt+"%'";
                pst=con.prepareStatement(query);
              rs=pst.executeQuery();
              tblsp.setModel(DbUtils.resultSetToTableModel(rs));
              }
               if (txt.isEmpty()) {
    String query = "SELECT * FROM `spare parts`";
    pst = con.prepareStatement(query);
    rs = pst.executeQuery();
    tblsp.setModel(DbUtils.resultSetToTableModel(rs));}
            }catch (SQLException ex) {
    ...
    private void showActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_showActionPerformed
        ...
        TableColumn hiddenColumn = tblsp.getColumnModel().getColumn(5);
        if (show.isSelected()) {
...
System.out.println("Button clicked");  
        ...
        if (selectedRow != -1) { // A row is selected  
            ...
            String itemName = tblsp.getValueAt(selectedRow, 1).toString();
            double price = Double.parseDouble(tblsp.getValueAt(selectedRow, 10).toString());
...
ArmatureFields af=new ArmatureFields();

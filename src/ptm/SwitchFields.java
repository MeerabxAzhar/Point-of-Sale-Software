...
public class SwitchFields extends javax.swing.JFrame {
Connection con;
 ...
 public void tbl() {
        try {
            pst = con.prepareStatement("select * from `switch`");
            rs = pst.executeQuery();
            ResultSetMetaData rsd = rs.getMetaData();
            d = (DefaultTableModel) tbls.getModel();
            d.setRowCount(0);
            while (rs.next()) {
                Vector v = new Vector();
            ...
            Logger.getLogger(SwitchFields.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
 public void Connect() {
        try {
    ...
    private void initComponents() {
        ...
        tbls.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Id", "Name", "Model No.", "Company", "Brand", "Cost Price", "Whole Price", "Retail Price", "Total Quantity", "Show Room", "Godam", "Total Cost"
            }
        ));
        tbls.setAutoResizeMode(javax.swing.JTable.AUTO_RESIZE_ALL_COLUMNS);
        tbls.setShowHorizontalLines(true);
        tbls.setShowVerticalLines(true);
        ...
        Id.setText("Id");
        jPanel2.add(Id, new org.netbeans.lib.awtextra.AbsoluteConstraints(270, 90, 40, 20));
        jPanel2.add(txtid, new org.netbeans.lib.awtextra.AbsoluteConstraints(330, 90, 160, 20));
    ...
    private void txtsearchKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtsearchKeyReleased
      String txt=txtsearch.getText();
          try {
              if(txt.matches("^[0-9]+$")){
            String query="SELECT * FROM `switch` Where `Model No`="+txt;
                pst=con.prepareStatement(query);
              rs=pst.executeQuery();
              tbls.setModel(DbUtils.resultSetToTableModel(rs));
              }
            else if (txt.matches("^[a-zA-Z0-9]+$")) {
    // Search by ID
    String query = "SELECT * FROM `switch` WHERE `Id` = '" + txt + "'";
    pst = con.prepareStatement(query);
              rs=pst.executeQuery();
              tbls.setModel(DbUtils.resultSetToTableModel(rs));
              }
              ...
              {
              String query="SELECT * FROM `switch` Where `Name` LIKE '%"+txt+"%'";
                pst=con.prepareStatement(query);
              rs=pst.executeQuery();
              tbls.setModel(DbUtils.resultSetToTableModel(rs));
              }
    ...
    private void btnaddMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnaddMouseClicked
         try {
            ...
            pst = con.prepareStatement("INSERT INTO `switch`(`Id`, `Name`, `Model No`, `Company`, `Brand`, `Cost Price`, `Whole Price`, `Retail Price`, `Total Quantity`, `Show Room`, `Godam`, `Total Cost`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)");
pst.setString(1, Id);
 ...
 String Id = txtid.getText();
            ...
            int TotalQuantity = Integer.parseInt(txtsr.getText()) + Integer.parseInt(txtgodam.getText());
            Double TotalCost = Double.parseDouble(txtcp.getText()) * TotalQuantity;
            pst = con.prepareStatement("UPDATE `switch` SET `Id`=?,`Name`=?,`Company`=?,`Brand`=?,`Cost Price`=?,`Whole Price`=?,`Retail Price`=?,`Total Quantity`=?,`Show Room`=?,`Godam`=?,`Total Cost`=? WHERE `Model No`=?");
           pst.setString(1, Id);
            pst.setString(2, ItemName);
             pst.setString(3, Company);
            pst.setString(4, Brand);
            pst.setString(12, ModelNo);
    ...
    private void tblsMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblsMouseClicked
          d = (DefaultTableModel) tbls.getModel();
        int sIndex = tbls.getSelectedRow();
...
System.out.println("Button clicked");  
        ...
        if (selectedRow != -1) { // A row is selected  
            String id = tbls.getValueAt(selectedRow, 0).toString();
            String itemName = tbls.getValueAt(selectedRow, 1).toString();
            double price = Double.parseDouble(tbls.getValueAt(selectedRow, 10).toString());
...
ot.setVisible(true);// TODO add your handling code here:
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(SwitchFields.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new SwitchFields().setVisible(true);
            }
        });
    }

...
public class ChiselFields extends javax.swing.JFrame {
 Connection con;
 ...
 public void tbl() {
        try {
            pst = con.prepareStatement("select * from `chisel`");
            rs = pst.executeQuery();
            ResultSetMetaData rsd = rs.getMetaData();
            d = (DefaultTableModel) tblcd.getModel();
            d.setRowCount(0);
            while (rs.next()) {
                Vector v = new Vector();
            ...
            Logger.getLogger(ChiselDetails.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
 ...
 public void Connect() {
    ...
    private void initComponents() {
        ...
        tblcd.setFont(new java.awt.Font("Times New Roman", 0, 14)); // NOI18N
        ...
        tblcd.setShowHorizontalLines(true);
        tblcd.setShowVerticalLines(true);
    ...
    private void txtsearchMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_txtsearchMouseClicked
       String txt=txtsearch.getText();
          try {
              if(txt.matches("^[0-9]+$")){
            String query="SELECT * FROM `chisel` Where `Model No`="+txt;
                pst=con.prepareStatement(query);
              rs=pst.executeQuery();
              tblcd.setModel(DbUtils.resultSetToTableModel(rs));
              }
            else if (txt.matches("^[a-zA-Z0-9]+$")) {
    // Search by ID
    String query = "SELECT * FROM `chisel` WHERE `Id` = '" + txt + "'";
    pst = con.prepareStatement(query);
              rs=pst.executeQuery();
              tblcd.setModel(DbUtils.resultSetToTableModel(rs));
              }
              ...
              {
              String query="SELECT * FROM `chisel` Where `Name` LIKE '%"+txt+"%'";
                pst=con.prepareStatement(query);
              rs=pst.executeQuery();
              tblcd.setModel(DbUtils.resultSetToTableModel(rs));
              }
               if (txt.isEmpty()) {
    String query = "SELECT * FROM `chisel`";
    pst = con.prepareStatement(query);
    rs = pst.executeQuery();
    tblcd.setModel(DbUtils.resultSetToTableModel(rs));}
            } catch (SQLException ex) {
                Logger.getLogger(ChiselFields.class.getName()).log(Level.SEVERE, null, ex);
            }
    }//GEN-LAST:event_txtsearchMouseClicked

    private void tblcdKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tblcdKeyReleased
        d = (DefaultTableModel) tblcd.getModel();
        int sIndex = tblcd.getSelectedRow();
    ...
    private void btnaddMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnaddMouseClicked
          try {
            ...
            pst = con.prepareStatement("INSERT INTO `chisel`(`Id`,`Name`, `Brand`, `Model No`, `Whole Sale`, `R Sale`, `Cost Price`, `Total Quantity`, `Show Room`, `Godam`, `Total Cost`) VALUES (?,?,?,?,?,?,?,?,?,?,?)");
pst.setString(1, Id);
 ...
 String Id = txtid.getText();
            ...
            int TotalQuantity = Integer.parseInt(txtsr.getText()) + Integer.parseInt(txtgodam.getText());
            Double TotalCost = Double.parseDouble(txtcp.getText()) * TotalQuantity;
            pst = con.prepareStatement("UPDATE `chisel` SET `Id`=?,`Name`=?,`Brand`=?,`Whole Sale`=?,`R Sale`=?,`Cost Price`=?,`Total Quantity`=?,`Show Room`=?,`Godam`=?,`Total Cost`=? WHERE `Model No`=?");
            pst.setString(1, Id);
            pst.setString(2, ItemName);
            pst.setString(3, Brand);
...
System.out.println("Button clicked");  
        ...
        if (selectedRow != -1) { // A row is selected  
            String id = tblcd.getValueAt(selectedRow, 0).toString();
            String itemName = tblcd.getValueAt(selectedRow, 1).toString();
            double price = Double.parseDouble(tblcd.getValueAt(selectedRow, 10).toString());
 ...
 Cart cart=new Cart();
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(ChiselFields.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new ChiselFields().setVisible(true);
            }
        });
    }

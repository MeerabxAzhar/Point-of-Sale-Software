...
public class Breakersfields extends javax.swing.JFrame {
Connection con;
 ...
 public void tbl() {
        try {
            pst = con.prepareStatement("select * from `breakers`");
            rs = pst.executeQuery();
 ...
 public void Connect() {
    ...
    private void initComponents() {
        ...
        jPanel1.add(btncart, new org.netbeans.lib.awtextra.AbsoluteConstraints(1110, 30, 40, -1));
    ...
    private void txtsearchKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtsearchKeyReleased
          ...
          try {
              if(txt.matches("^[0-9]+$")){
            String query="SELECT * FROM `breakers` Where `Model No`="+txt;
    ...
    // Search by ID
    String query = "SELECT * FROM `breakers` WHERE `Id` = '" + txt + "'";
    pst = con.prepareStatement(query);
              rs=pst.executeQuery();
              tblb.setModel(DbUtils.resultSetToTableModel(rs));
              }
              ...
              {
              String query="SELECT * FROM `breakers` Where `Item Name` LIKE '%"+txt+"%'";
                pst=con.prepareStatement(query);
              rs=pst.executeQuery();
              tblb.setModel(DbUtils.resultSetToTableModel(rs));
              }
               if (txt.isEmpty()) {
    String query = "SELECT * FROM `breakers`";
    pst = con.prepareStatement(query);
    rs = pst.executeQuery();
    tblb.setModel(DbUtils.resultSetToTableModel(rs));}
          } catch (SQLException ex) {
    ...
    private void btnaddMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnaddMouseClicked
        try {
            ...
            pst = con.prepareStatement("INSERT INTO `breakers`(`Id`, `Name`, `Model No`, `Company`, `Brand`, `Cost Price`, `Whole Price`, `Retail Price`, `Total Quantity`, `Show Room`, `Godam`, `Total Cost`, `Details`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)");
pst.setString(1, Id);
            ...
            Logger.getLogger(Breakersfields.class.getName()).log(Level.SEVERE, null, ex);
        }
 ...
 String Id = txtid.getText();
            ...
            int TotalQuantity = Integer.parseInt(txtsr.getText()) + Integer.parseInt(txtgodam.getText());
            Double TotalCost = Double.parseDouble(txtcp.getText()) * TotalQuantity;
            pst = con.prepareStatement("UPDATE `breakers` SET `Id`=?,`Name`=?,`Company`=?,`Brand`=?,`Cost Price`=?,`Whole Price`=?,`Retail Price`=?,`Total Quantity`=?,`Show Room`=?,`Godam`=?,`Total Cost`=?,`Details`=? WHERE `Model No`=?");
           pst.setString(1, Id);
            pst.setString(2, ItemName);
 ...
 Cart cart=new Cart();
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(Breakersfields.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new Breakersfields().setVisible(true);
            }
        });
    }

...
Connection con;
 ...
 public void tbl() {
        try {
            ...
            d = (DefaultTableModel) tblbf.getModel();
            d.setRowCount(0);
            while (rs.next()) {
                Vector v = new Vector();
             ...
             TableColumn column = tblbf.getColumnModel().getColumn(6);
 ...
 public void Connect() {
    ...
    private void initComponents() {
        ...
        tblbf.setFont(new java.awt.Font("Times New Roman", 0, 14)); // NOI18N
        tblbf.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
        ...
        tblbf.setShowHorizontalLines(true);
        tblbf.setShowVerticalLines(true);
        jScrollPane1.setViewportView(tblbf);
    ...
    String txt=txtsearch.getText();
          try {
              if(txt.matches("^[0-9]+$")){
            String query="SELECT * FROM `accessories` Where `Model No`="+txt;
                pst=con.prepareStatement(query);
              rs=pst.executeQuery();
              tblbf.setModel(DbUtils.resultSetToTableModel(rs));
              }
            else if (txt.matches("^[a-zA-Z0-9]+$")) {
    // Search by ID
    String query = "SELECT * FROM `accessories` WHERE `Id` = '" + txt + "'";
    pst = con.prepareStatement(query);
              rs=pst.executeQuery();
              tblbf.setModel(DbUtils.resultSetToTableModel(rs));
              }
              ...
              {
              String query="SELECT * FROM `accessories` Where `Name` LIKE '%"+txt+"%'";
                pst=con.prepareStatement(query);
              rs=pst.executeQuery();
              tblbf.setModel(DbUtils.resultSetToTableModel(rs));
              }
    ...
    private void showActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_showActionPerformed
        ...
        TableColumn hiddenColumn = tblbf.getColumnModel().getColumn(6);
        if (show.isSelected()) {
  ...
  System.out.println("Button clicked");  
        int selectedRow = tblbf.getSelectedRow();
        if (selectedRow != -1) { // A row is selected  
            String id = tblbf.getValueAt(selectedRow, 0).toString();
            String itemName = tblbf.getValueAt(selectedRow, 1).toString();
            double price = Double.parseDouble(tblbf.getValueAt(selectedRow, 10).toString());
...
BladesFields bf=new BladesFields();
bf.setVisible(true);        // TODO add your handling code here:
    ...
    private javax.swing.JTable tblbf;

...
public class KeysFields extends javax.swing.JFrame {
Connection con;
 ...
 public void tbl() {
        try {
            pst = con.prepareStatement("select * from `keys`");
            rs = pst.executeQuery();
            ResultSetMetaData rsd = rs.getMetaData();
            d = (DefaultTableModel) tblk.getModel();
            d.setRowCount(0);
            while (rs.next()) {
                Vector v = new Vector();
            ...
            Logger.getLogger(KeysFields.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
 public void Connect() {
        try {
    ...
    private void initComponents() {
        ...
        tblk.setFont(new java.awt.Font("Times New Roman", 0, 14)); // NOI18N
        ...
        tblk.setShowHorizontalLines(true);
        tblk.setShowVerticalLines(true);
    ...
    private void txtsearchKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtsearchKeyReleased
         String txt=txtsearch.getText();
          try {
              if(txt.matches("^[0-9]+$")){
            String query="SELECT * FROM `keys` Where `Model No`="+txt;
                pst=con.prepareStatement(query);
              rs=pst.executeQuery();
              tblk.setModel(DbUtils.resultSetToTableModel(rs));
              }
            else if (txt.matches("^[a-zA-Z0-9]+$")) {
    // Search by ID
    String query = "SELECT * FROM `keys` WHERE `Id` = '" + txt + "'";
    pst = con.prepareStatement(query);
              rs=pst.executeQuery();
              tblk.setModel(DbUtils.resultSetToTableModel(rs));
              }
              ...
              {
              String query="SELECT * FROM `keys` Where `Name` LIKE '%"+txt+"%'";
                pst=con.prepareStatement(query);
              rs=pst.executeQuery();
              tblk.setModel(DbUtils.resultSetToTableModel(rs));
              }
    ...
    private void btnaddMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnaddMouseClicked
          try {
            ...
            pst = con.prepareStatement("INSERT INTO `keys`(`Id`,`Name`, `Brand`, `Model No`, `Whole Sale`, `R Sale`, `Cost Price`, `Total Quantity`, `Show Room`, `Godam`, `Total Cost`) VALUES (?,?,?,?,?,?,?,?,?,?,?)");
pst.setString(1, Id);
 ...
 String Id = txtid.getText();
            ...
            int TotalQuantity = Integer.parseInt(txtsr.getText()) + Integer.parseInt(txtgodam.getText());
            Double TotalCost = Double.parseDouble(txtcp.getText()) * TotalQuantity;
            pst = con.prepareStatement("UPDATE `keys` SET `Id`=?,`Name`=?,`Brand`=?,`Whole Sale`=?,`R Sale`=?,`Cost Price`=?,`Total Quantity`=?,`Show Room`=?,`Godam`=?,`Total Cost`=? WHERE `Model No`=?");
            pst.setString(1, Id);
            pst.setString(2, ItemName);
            pst.setString(3, Brand);
    ...
    private void tblkMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblkMouseClicked
         d = (DefaultTableModel) tblk.getModel();
        int sIndex = tblk.getSelectedRow();
...
System.out.println("Button clicked");  
        int selectedRow = tblk.getSelectedRow();
        if (selectedRow != -1) { // A row is selected  
            String id = tblk.getValueAt(selectedRow, 0).toString();
            String itemName = tblk.getValueAt(selectedRow, 1).toString();
            double price = Double.parseDouble(tblk.getValueAt(selectedRow, 10).toString());
...
k.setVisible(true);         // TODO add your handling code here:
    ...
    private void btnweldingMouseEntered(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnweldingMouseEntered
        // TODO add your handling code here:
    }//GEN-LAST:event_btnweldingMouseEntered
...
wed.setVisible(true);        // TODO add your handling code here:
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(KeysFields.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new KeysFields().setVisible(true);
            }
        });
    }

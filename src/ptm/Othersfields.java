...
public class Othersfields extends javax.swing.JFrame {
Connection con;
 ...
 public void tbl() {
        try {
            ...
            d = (DefaultTableModel) tblo.getModel();
            d.setRowCount(0);
            while (rs.next()) {
                Vector v = new Vector();
            ...
            Logger.getLogger(Othersfields.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
 public void Connect() {
        try {
    ...
    private void initComponents() {
        ...
        tblo.setFont(new java.awt.Font("Times New Roman", 0, 14)); // NOI18N
        ...
        tblo.setAutoResizeMode(javax.swing.JTable.AUTO_RESIZE_ALL_COLUMNS);
        tblo.setShowHorizontalLines(true);
        tblo.setShowVerticalLines(true);
        tblo.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                tbloMouseClicked(evt);
            }
        });
        jScrollPane2.setViewportView(tblo);
    ...
    private void txtsearchKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtsearchKeyReleased
          ...
          try {
              if(txt.matches("^[0-9]+$")){
            String query="SELECT * FROM `others` Where `Model No`="+txt;
                pst=con.prepareStatement(query);
              rs=pst.executeQuery();
              tblo.setModel(DbUtils.resultSetToTableModel(rs));
              }
            else if (txt.matches("^[a-zA-Z0-9]+$")) {
    // Search by ID
    String query = "SELECT * FROM `others` WHERE `Id` = '" + txt + "'";
    pst = con.prepareStatement(query);
              rs=pst.executeQuery();
              tblo.setModel(DbUtils.resultSetToTableModel(rs));
              }
              ...
              {
              String query="SELECT * FROM `others` Where `Item Name` LIKE '%"+txt+"%'";
                pst=con.prepareStatement(query);
              rs=pst.executeQuery();
              tblo.setModel(DbUtils.resultSetToTableModel(rs));
              }
    ...
    private void tbloMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbloMouseClicked
        d = (DefaultTableModel) tblo.getModel();
        int sIndex = tblo.getSelectedRow();
    ...
    private void btnaddMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnaddMouseClicked
        try {
            ...
            pst = con.prepareStatement("INSERT INTO `others`(`Id`, `Name`, `Model No`, `Company`, `Brand`, `Cost Price`, `Whole Price`, `Retail Price`, `Total Quantity`, `Show Room`, `Godam`, `Total Cost`, `Details`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)");
pst.setString(1, Id);
 ...
 String Id = txtid.getText();
            ...
            int TotalQuantity = Integer.parseInt(txtsr.getText()) + Integer.parseInt(txtgodam.getText());
            Double TotalCost = Double.parseDouble(txtcp.getText()) * TotalQuantity;
            pst = con.prepareStatement("UPDATE `others` SET `Id`=?,`Name`=?,`Company`=?,`Brand`=?,`Cost Price`=?,`Whole Price`=?,`Retail Price`=?,`Total Quantity`=?,`Show Room`=?,`Godam`=?,`Total Cost`=?,`Details`=? WHERE `Model No`=?");
           pst.setString(1, Id);
            pst.setString(2, ItemName);
...
System.out.println("Button clicked");  
        int selectedRow = tblo.getSelectedRow();
        if (selectedRow != -1) { // A row is selected  
            String id = tblo.getValueAt(selectedRow, 0).toString();
            String itemName = tblo.getValueAt(selectedRow, 1).toString();
            double price = Double.parseDouble(tblo.getValueAt(selectedRow, 10).toString());
...
ot.setVisible(true);         // TODO add your handling code here:
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(Othersfields.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new Othersfields().setVisible(true);
            }
        });
    }

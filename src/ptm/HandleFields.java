...
public class HandleFields extends javax.swing.JFrame {
Connection con;
 ...
 public void tbl() {
        try {
            pst = con.prepareStatement("select * from `handle`");
            rs = pst.executeQuery();
            ResultSetMetaData rsd = rs.getMetaData();
            d = (DefaultTableModel) tblh.getModel();
            d.setRowCount(0);
            while (rs.next()) {
                Vector v = new Vector();
            ...
            Logger.getLogger(HandleFields.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
 ...
 public void Connect() {
    ...
    private void initComponents() {
        ...
        jPanel5.add(btnbill, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 140, 130, 40));
        ...
        jPanel5.add(btnadd, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 40, 130, 40));
        ...
        jPanel5.add(btnupdate, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 90, 130, 40));

        getContentPane().add(jPanel5, new org.netbeans.lib.awtextra.AbsoluteConstraints(1110, 60, 130, 630));
        ...
        tblh.setFont(new java.awt.Font("Times New Roman", 0, 14)); // NOI18N
        ...
        tblh.setShowHorizontalLines(true);
        tblh.setShowVerticalLines(true);
        tblh.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                tblhMouseClicked(evt);
            }
        });
    ...
    private void txtsearchKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtsearchKeyReleased
          ...
          try {
              if(txt.matches("^[0-9]+$")){
            String query="SELECT * FROM `handle` Where `Model No`="+txt;
                pst=con.prepareStatement(query);
              rs=pst.executeQuery();
              tblh.setModel(DbUtils.resultSetToTableModel(rs));
              }
            else if (txt.matches("^[a-zA-Z0-9]+$")) {
    // Search by ID
    String query = "SELECT * FROM `handle` WHERE `Id` = '" + txt + "'";
    pst = con.prepareStatement(query);
              rs=pst.executeQuery();
              tblh.setModel(DbUtils.resultSetToTableModel(rs));
              }
              ...
              {
              String query="SELECT * FROM `handle` Where `Name` LIKE '%"+txt+"%'";
                pst=con.prepareStatement(query);
              rs=pst.executeQuery();
              tblh.setModel(DbUtils.resultSetToTableModel(rs));
              }
    ...
    private void tblhMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblhMouseClicked
         d = (DefaultTableModel) tblh.getModel();
        int sIndex = tblh.getSelectedRow();
    ...
    }//GEN-LAST:event_tblhMouseClicked
    ...
    private void btnaddMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnaddMouseClicked
          try {
            ...
            pst = con.prepareStatement("INSERT INTO `handle`(`Id`,`Name`, `Brand`, `Model No`, `Whole Sale`, `R Sale`, `Cost Price`, `Total Quantity`, `Show Room`, `Godam`, `Total Cost`) VALUES (?,?,?,?,?,?,?,?,?,?,?)");
pst.setString(1, Id);
 ...
 String Id = txtid.getText();
            ...
            int TotalQuantity = Integer.parseInt(txtsr.getText()) + Integer.parseInt(txtgodam.getText());
            Double TotalCost = Double.parseDouble(txtcp.getText()) * TotalQuantity;
            pst = con.prepareStatement("UPDATE `handle` SET `Id`=?,`Name`=?,`Brand`=?,`Whole Sale`=?,`R Sale`=?,`Cost Price`=?,`Total Quantity`=?,`Show Room`=?,`Godam`=?,`Total Cost`=? WHERE `Model No`=?");
            pst.setString(1, Id);
            pst.setString(2, ItemName);
            pst.setString(3, Brand);
...
System.out.println("Button clicked");  
        int selectedRow = tblh.getSelectedRow();
        if (selectedRow != -1) { // A row is selected  
            String id = tblh.getValueAt(selectedRow, 0).toString();
            String itemName = tblh.getValueAt(selectedRow, 1).toString();
            double price = Double.parseDouble(tblh.getValueAt(selectedRow, 10).toString());
...
ht.setVisible(true);          // TODO add your handling code here:
    ...
    private void lblaccessMouseEntered(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_lblaccessMouseEntered
        // TODO add your handling code here:
    }//GEN-LAST:event_lblaccessMouseEntered
...
wed.setVisible(true);              // TODO add your handling code here:
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(HandleFields.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new HandleFields().setVisible(true);
            }
        });
    }

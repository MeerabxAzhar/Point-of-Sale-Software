...
public class FieldFields extends javax.swing.JFrame {
Connection con;
 ...
 public void tbl() {
        try {
            pst = con.prepareStatement("select * from `field`");
            rs = pst.executeQuery();
            ResultSetMetaData rsd = rs.getMetaData();
            d = (DefaultTableModel) tblf.getModel();
            d.setRowCount(0);
            while (rs.next()) {
                Vector v = new Vector();
                ...
                v.add(rs.getString("Core ID"));
                v.add(rs.getString("Core O.D"));
                v.add(rs.getString("Core Length"));
                d.addRow(v);
            }
 ...
 public void Connect() {
    ...
    private void initComponents() {
        ...
        cid.setText("Core I.D");
        jPanel2.add(cid, new org.netbeans.lib.awtextra.AbsoluteConstraints(260, 90, 70, -1));

        cl.setFont(new java.awt.Font("Times New Roman", 0, 14)); // NOI18N
        cl.setText("Core Length");
        jPanel2.add(cl, new org.netbeans.lib.awtextra.AbsoluteConstraints(750, 90, 70, -1));

        cod.setFont(new java.awt.Font("Times New Roman", 0, 14)); // NOI18N
        cod.setText("Core O.D");
        jPanel2.add(cod, new org.netbeans.lib.awtextra.AbsoluteConstraints(500, 90, 70, -1));
        jPanel2.add(txtcod, new org.netbeans.lib.awtextra.AbsoluteConstraints(580, 90, 160, -1));
        jPanel2.add(txtcl, new org.netbeans.lib.awtextra.AbsoluteConstraints(830, 90, 160, -1));
        jPanel2.add(txtgodam, new org.netbeans.lib.awtextra.AbsoluteConstraints(90, 90, 160, -1));
        jPanel2.add(txtcid, new org.netbeans.lib.awtextra.AbsoluteConstraints(330, 90, 160, -1));

        tblf.setFont(new java.awt.Font("Times New Roman", 0, 14)); // NOI18N
        tblf.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Id", "Name", "Model No.", "Company", "Brand", "C.P", "W.P", "R.P", "T.Q", "S.R", "G.D", "T.C", "Core ID", "Core O.D", "Core Length"
            }
        ));
        tblf.setAutoResizeMode(javax.swing.JTable.AUTO_RESIZE_ALL_COLUMNS);
        tblf.setShowHorizontalLines(true);
        tblf.setShowVerticalLines(true);
        ...
        Id.setText("Id");
        jPanel2.add(Id, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 130, 30, 20));
    ...
    private void txtsearchKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtsearchKeyReleased
         String txt=txtsearch.getText();
          try {
              if(txt.matches("^[0-9]+$")){
            String query="SELECT * FROM `field` Where `Model No`="+txt;
                pst=con.prepareStatement(query);
              rs=pst.executeQuery();
              tblf.setModel(DbUtils.resultSetToTableModel(rs));
              }
            else if (txt.matches("^[a-zA-Z0-9]+$")) {
    // Search by ID
    String query = "SELECT * FROM `field` WHERE `Id` = '" + txt + "'";
    pst = con.prepareStatement(query);
              rs=pst.executeQuery();
              tblf.setModel(DbUtils.resultSetToTableModel(rs));
              }
              ...
              {
              String query="SELECT * FROM `field` Where `Name` LIKE '%"+txt+"%'";
                pst=con.prepareStatement(query);
              rs=pst.executeQuery();
              tblf.setModel(DbUtils.resultSetToTableModel(rs));
              }
    ...
    private void tblfMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblfMouseClicked
         d = (DefaultTableModel) tblf.getModel();
        int sIndex = tblf.getSelectedRow();
        ...
        txtcid.setText(d.getValueAt(sIndex, 12).toString());
        txtcod.setText(d.getValueAt(sIndex, 13).toString());
        txtcl.setText(d.getValueAt(sIndex, 14).toString());
    ...
    private void btnaddMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnaddMouseClicked
        try {
            ...
            String CoreID = txtcid.getText();
            String CoreOD = txtcod.getText();
            String CoreLength = txtcl.getText();
            ...
            pst = con.prepareStatement("INSERT INTO `field`(`Id`, `Name`, `Model No`, `Company`, `Brand`, `C.P`, `W.P`, `R.P`, `Total Quantity`, `Show Room`, `G.D`, `T.C`, `Core ID`, `Core O.D`, `Core Length`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
            pst.setString(1, Id);
            ...
            Logger.getLogger(FieldFields.class.getName()).log(Level.SEVERE, null, ex);
        }
  ...
  String Id = txtid.getText();
            ...
            int TotalQuantity = Integer.parseInt(txtsr.getText()) + Integer.parseInt(txtgodam.getText());
            Double TotalCost = Double.parseDouble(txtcp.getText()) * TotalQuantity;
            pst = con.prepareStatement("UPDATE `field` SET `Id`=?,`Name`=?,`Company`=?,`Brand`=?,`C.P`=?,`W.P`=?,`R.P`=?,`Total Quantity`=?,`Show Room`=?,`G.D`=?,`T.C`=?,`Core ID`=?,`Core O.D`=?,`Core Length`=? WHERE `Model No`=?");
             pst.setString(1, Id);
            pst.setString(2, Name);
...
System.out.println("Button clicked");  
        ...
        if (selectedRow != -1) { // A row is selected  
            ...
            String itemName = tblf.getValueAt(selectedRow, 1).toString();
            double price = Double.parseDouble(tblf.getValueAt(selectedRow, 10).toString());
 ...
 Cart cart=new Cart();
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(FieldFields.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new FieldFields().setVisible(true);
            }
        });
    }

...
public class PTAdd extends javax.swing.JFrame {
...
Cart cart=new Cart();
    public void tbl() {
        try {
            pst = con.prepareStatement("select * from powertolls");
            rs = pst.executeQuery();
            ResultSetMetaData rsd = rs.getMetaData();
            d = (DefaultTableModel) tblpt.getModel();
            d.setRowCount(0);
            while (rs.next()) {
                Vector v = new Vector();
    ...
    private void initComponents() {
        ...
        tblpt.setFont(new java.awt.Font("Times New Roman", 0, 14)); // NOI18N
        ...
        tblpt.setShowHorizontalLines(true);
        tblpt.setShowVerticalLines(true);
        ...
        jPanel2.add(jScrollPane1, new org.netbeans.lib.awtextra.AbsoluteConstraints(30, 210, 1000, 370));
        ...
        jPanel2.add(show, new org.netbeans.lib.awtextra.AbsoluteConstraints(980, 180, 50, 20));
        ...
        jPanel1.add(btncart, new org.netbeans.lib.awtextra.AbsoluteConstraints(1110, 30, -1, -1));
    ...
    private void btnaddMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnaddMouseClicked
        try {
            ...
            pst = con.prepareStatement("INSERT INTO `powertolls`(`Id`,`Name`, `Brand`, `Model No`, `Whole Sale`, `R Sale`, `Cost Price`, `Total Quantity`, `Show Room`, `Godam`, `Total Cost`) VALUES (?,?,?,?,?,?,?,?,?,?,?)");
 pst.setString(1, Id);
...
String Id = txtid.getText();
            ...
            int TotalQuantity = Integer.parseInt(txtsr.getText()) + Integer.parseInt(txtgodam.getText());
            Double TotalCost = Double.parseDouble(txtcp.getText()) * TotalQuantity;
            pst = con.prepareStatement("UPDATE `powertolls` SET `Id`=?,`Name`=?,`Brand`=?,`Whole Sale`=?,`R Sale`=?,`Cost Price`=?,`Total Quantity`=?,`Show Room`=?,`Godam`=?,`Total Cost`=? WHERE `Model No`=?");
            pst.setString(1, Id);
            pst.setString(2, ItemName);
            pst.setString(3, Brand);
    ...
    private void tblptMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblptMouseClicked
        d = (DefaultTableModel) tblpt.getModel();
        int sIndex = tblpt.getSelectedRow();
    ...
    private void txtsearchKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtsearchKeyReleased
          ...
          try {
              if(txt.matches("^[0-9]+$")){
            String query="SELECT * FROM powertolls Where `Model No`="+txt;
                pst=con.prepareStatement(query);
              rs=pst.executeQuery();
              tblpt.setModel(DbUtils.resultSetToTableModel(rs));
              }
            else if (txt.matches("^[a-zA-Z0-9]+$")) {
    // Search by ID
    String query = "SELECT * FROM powertolls WHERE `Id` = '" + txt + "'";
    pst = con.prepareStatement(query);
              rs=pst.executeQuery();
              tblpt.setModel(DbUtils.resultSetToTableModel(rs));
              }
              ...
              {
              String query="SELECT * FROM powertolls Where `Name` LIKE '%"+txt+"%'";
                pst=con.prepareStatement(query);
              rs=pst.executeQuery();
              tblpt.setModel(DbUtils.resultSetToTableModel(rs));
              }
               if (txt.isEmpty()) {
    String query = "SELECT * FROM powertolls";
    pst = con.prepareStatement(query);
    rs = pst.executeQuery();
    tblpt.setModel(DbUtils.resultSetToTableModel(rs));}
            } catch (SQLException ex) {
...
System.out.println("Button clicked");  
        int selectedRow = tblpt.getSelectedRow();
        if (selectedRow != -1) { // A row is selected  
            String id = tblpt.getValueAt(selectedRow, 0).toString();
            String itemName = tblpt.getValueAt(selectedRow, 1).toString();
            double price = Double.parseDouble(tblpt.getValueAt(selectedRow, 10).toString());
...
wra.generateWarningReport();          // TODO add your handling code here:
 ...
 Cart cart=new Cart();
    ...
    private void showActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_showActionPerformed
        ...
        TableColumn hiddenColumn = tblpt.getColumnModel().getColumn(6);
        if (show.isSelected()) {
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(PTAdd.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);

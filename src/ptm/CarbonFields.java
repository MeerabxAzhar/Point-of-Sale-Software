...
public class CarbonFields extends javax.swing.JFrame {
Connection con;
 ...
 public void tbl() {
        try {
            pst = con.prepareStatement("select * from `carbon`");
            rs = pst.executeQuery();
            ResultSetMetaData rsd = rs.getMetaData();
            d = (DefaultTableModel) tblc.getModel();
            d.setRowCount(0);
            while (rs.next()) {
                Vector v = new Vector();
                ...
                v.add(rs.getString("Length"));
                v.add(rs.getString("Width"));
                v.add(rs.getString("Thickness"));
                d.addRow(v);
            }
 ...
 public void Connect() {
        try {
            ...
            try {
                ...
                Logger.getLogger(CarbonFields.class.getName()).log(Level.SEVERE, null, ex);
            }
    ...
    private void initComponents() {
        ...
        thickness = new javax.swing.JLabel();
        ...
        jPanel2.add(length, new org.netbeans.lib.awtextra.AbsoluteConstraints(260, 90, 70, -1));

        thickness.setFont(new java.awt.Font("Times New Roman", 0, 14)); // NOI18N
        thickness.setText("Thickness");
        jPanel2.add(thickness, new org.netbeans.lib.awtextra.AbsoluteConstraints(750, 90, 70, -1));

        width.setFont(new java.awt.Font("Times New Roman", 0, 14)); // NOI18N
        width.setText("Width");
        jPanel2.add(width, new org.netbeans.lib.awtextra.AbsoluteConstraints(500, 90, 70, -1));
        jPanel2.add(txtwidth, new org.netbeans.lib.awtextra.AbsoluteConstraints(580, 90, 160, -1));
        jPanel2.add(txtthickness, new org.netbeans.lib.awtextra.AbsoluteConstraints(830, 90, 160, -1));
        jPanel2.add(txtgodam, new org.netbeans.lib.awtextra.AbsoluteConstraints(90, 90, 160, -1));
        jPanel2.add(txtlength, new org.netbeans.lib.awtextra.AbsoluteConstraints(330, 90, 160, -1));

        tblc.setFont(new java.awt.Font("Times New Roman", 0, 14)); // NOI18N
        tblc.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Id", "Name", "Model No.", "Company", "Brand", "C.P", "W.P", "R.P", "T.Q", "S.R", "G.D", "T.C", "Length", "Width", "Thickness"
            }
        ));
        tblc.setAutoResizeMode(javax.swing.JTable.AUTO_RESIZE_ALL_COLUMNS);
        tblc.setShowHorizontalLines(true);
        tblc.setShowVerticalLines(true);
        ...
        jPanel2.add(jScrollPane2, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 160, 1000, 460));
        ...
        jPanel2.add(txtid, new org.netbeans.lib.awtextra.AbsoluteConstraints(90, 130, 160, -1));
    ...
    private void txtsearchKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtsearchKeyReleased
          ...
          try {
              if(txt.matches("^[0-9]+$")){
            String query="SELECT * FROM `carbon` Where `Model No`="+txt;
                pst=con.prepareStatement(query);
              rs=pst.executeQuery();
              tblc.setModel(DbUtils.resultSetToTableModel(rs));
              }
            else if (txt.matches("^[a-zA-Z0-9]+$")) {
    // Search by ID
    String query = "SELECT * FROM `carbon` WHERE `Id` = '" + txt + "'";
    pst = con.prepareStatement(query);
              rs=pst.executeQuery();
              tblc.setModel(DbUtils.resultSetToTableModel(rs));
              }
              ...
              {
              String query="SELECT * FROM `carbon` Where `Name` LIKE '%"+txt+"%'";
                pst=con.prepareStatement(query);
              rs=pst.executeQuery();
              tblc.setModel(DbUtils.resultSetToTableModel(rs));
              }
               if (txt.isEmpty()) {
    String query = "SELECT * FROM `carbon`";
    pst = con.prepareStatement(query);
    rs = pst.executeQuery();
    tblc.setModel(DbUtils.resultSetToTableModel(rs));}
          } catch (SQLException ex) {
    ...
    private void tblcMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblcMouseClicked
        d = (DefaultTableModel) tblc.getModel();
        int sIndex = tblc.getSelectedRow();
        ...
        txtlength.setText(d.getValueAt(sIndex, 12).toString());
        txtwidth.setText(d.getValueAt(sIndex, 13).toString());
        txtthickness.setText(d.getValueAt(sIndex, 14).toString());
    ...
    private void btnaddMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnaddMouseClicked
     try {
            ...
            String CoreID = txtlength.getText();
            String CoreOD = txtwidth.getText();
            String CoreLength = txtthickness.getText();
            ...
            pst = con.prepareStatement("INSERT INTO `carbon`(`Id`, `Name`, `Model No`, `Company`, `Brand`, `C.P`, `W.P`, `R.P`, `Total Quantity`, `Show Room`, `G.D`, `T.C`, `Length`, `Width`, `Thickness`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
            pst.setString(1, Id);
  ...
  String Id = txtid.getText();
            ...
            int TotalQuantity = Integer.parseInt(txtsr.getText()) + Integer.parseInt(txtgodam.getText());
            Double TotalCost = Double.parseDouble(txtcp.getText()) * TotalQuantity;
            pst = con.prepareStatement("UPDATE `carbon` SET `Id`=?,`Name`=?,`Company`=?,`Brand`=?,`C.P`=?,`W.P`=?,`R.P`=?,`Total Quantity`=?,`Show Room`=?,`G.D`=?,`T.C`=?,`Length`=?,`Width`=?,`Thickness`=? WHERE `Model No`=?");
             pst.setString(1, Id);
            pst.setString(2, Name);
...
System.out.println("Button clicked");  
        ...
        if (selectedRow != -1) { // A row is selected  
            ...
            String itemName = tblc.getValueAt(selectedRow, 1).toString();
            double price = Double.parseDouble(tblc.getValueAt(selectedRow, 10).toString());
 ...
 Cart cart=new Cart();
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(CarbonFields.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new CarbonFields().setVisible(true);
            }
        });
    }
    ...
    private javax.swing.JLabel thickness;
